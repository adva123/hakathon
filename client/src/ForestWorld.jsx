...
export function ForestWorld({ floorY, curveData, robotRef, gestureRef, roomPortals, completion, onPortalEnter }) {
  // Get the active overlay room from GameContext
  const { activeOverlayRoom } = useContext(GameContext);
  ...
  return (
    <group>
      ...
      {/* 3D Password Room - Underground Bunker */}
      {portals.length > 0 && activeOverlayRoom === SCENES.password ? (
        (() => {
          const passwordPortal = portals.find(p => p.scene === 'password');
          if (!passwordPortal?.platform) {
            console.log('❌ Password portal not found or no platform');
            return null;
          }
          
          console.log('✅ Showing 3D Password Room at:', passwordPortal.platform);
          
          return (
            <PasswordRoom3D
              position={[
                passwordPortal.platform.x,
                passwordPortal.platform.y + 0.5,
